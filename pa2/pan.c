...
short src_ln0 [] = {
	  0,  18,  18,  19,  20,  21,  23,  23, 
	 24,  22,  26,  26,  28,  29,  30,  31, 
	 17,  33,  17,  33,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "alt.pml", 1, 19 },
...
{	int j = 0, h = now._nr_pr;
	...
	case 0:	/* pi */
		...
		((P0 *)pptr(h))->max = par1;
		/* locals: */
		((P0 *)pptr(h))->occupied = 0;
#ifdef VAR_RANGES
		logval("pi:i", ((P0 *)pptr(h))->i);
		logval("pi:max", ((P0 *)pptr(h))->max);
		logval("pi:occupied", ((P0 *)pptr(h))->occupied);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in; /* 2 */
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("p_status[l_in]", now.p_status[l_in]);
		}
	}
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  IDLE:	3\n");
	printf("	mtype  REQUESTING:	2\n");
	printf("	mtype  RUNNING:	1\n");
	{	int l_in; /* 1 */
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	mtype  p_status[%d]:	%d\n", l_in, now.p_status[l_in]);
		}
	}
	printf("	short  available:	%d\n", now.available);
}
void
...
{	/* int i; */
	...
	printf("	short  i:	%d\n", ((P0 *)pptr(pid))->i);
	printf("	short  max:	%d\n", ((P0 *)pptr(pid))->max);
	printf("	short  occupied:	%d\n", ((P0 *)pptr(pid))->occupied);
		break;
	}
}
void
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("RUNNING"); return;
	case 2: Printf("REQUESTING"); return;
